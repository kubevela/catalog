...
EXPERIMENTAL_ADDONS=`ls -l experimental/addons | grep "^d" | awk '{print $9}' | sort`

echo $EXPERIMENTAL_ADDONS

for i in $EXPERIMENTAL_ADDONS ; do

version=$(cat experimental/addons/$i/metadata.yaml | grep "version"| awk '{print $2}')
